...
// 데스크탑 앱이 발급한 API 토큰(preload) — 패키징 앱의 백엔드는 이 헤더 없는 요청을 거부한다.
// 브라우저 dev(토큰 없음)에선 빈 값 → 헤더 생략, 백엔드도 강제 안 함.
const TOKEN: string = (typeof window !== "undefined" && (window as any).ghost?.token) || "";
function ghostFetch(input: string, init: RequestInit = {}): Promise<Response> {
  if (!TOKEN) return fetch(input, init);
  const headers = new Headers(init.headers || {});
  headers.set("x-ghost-token", TOKEN);
...
export async function getStatus(): Promise<Status> {
  const r = await ghostFetch(`${BASE}/api/status`);
  if (!r.ok) throw new Error("status failed");
  return r.json();
}

export async function setBackend(backend: string): Promise<Status> {
  const r = await ghostFetch(`${BASE}/api/config`, {
    method: "POST",
...
export async function route(text: string, context = "", meetingId = ""): Promise<Route> {
  const r = await ghostFetch(`${BASE}/api/route`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function startMeeting(): Promise<MeetingMeta> {
  const r = await ghostFetch(`${BASE}/api/meetings`, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
  if (!r.ok) throw new Error("start meeting failed");
  return (await r.json()).meeting;
}
export async function endMeeting(meetingId: string): Promise<void> {
  try { await ghostFetch(`${BASE}/api/meetings/${meetingId}/end`, { method: "POST" }); } catch { /* ignore */ }
}
export async function setMeetingTitle(meetingId: string, title: string): Promise<MeetingMeta | null> {
  try {
    const r = await ghostFetch(`${BASE}/api/meetings/${meetingId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title }) });
...
export async function setMeetingFolder(meetingId: string, folder: string): Promise<MeetingMeta | null> {
  try {
    const r = await ghostFetch(`${BASE}/api/meetings/${meetingId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ folder }) });
    return (await r.json()).meeting ?? null;
  } catch { return null; }
}
export async function getMeeting(meetingId: string): Promise<any | null> {
  try { return (await (await ghostFetch(`${BASE}/api/meetings/${meetingId}`)).json()).meeting ?? null; } catch { return null; }
}
/** 회의 영구 삭제(전사·회의록·다이제스트 포함). */
export async function deleteMeeting(meetingId: string): Promise<boolean> {
  try { return (await (await ghostFetch(`${BASE}/api/meetings/${meetingId}`, { method: "DELETE" })).json()).ok === true; } catch { return false; }
}
/** 저장된 5분 다이제스트 기록(재시작 후 복원용). */
...
export async function setMeetingContext(meetingId: string, context: string): Promise<boolean> {
  try {
    const r = await ghostFetch(`${BASE}/api/meetings/${meetingId}/context`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ context }) });
...
function streamSSE(path: string, body: object, h: StreamHandlers): () => void {
  ...
  (async () => {
    const r = await ghostFetch(`${BASE}${path}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function getConnectors(): Promise<{ name: string; status: string }[]> {
  try { return (await (await ghostFetch(`${BASE}/api/connectors`)).json()).connectors || []; } catch { return []; }
}
...
export async function getConnectorRegistry(): Promise<{ connectors: RegistryConnector[]; custom: string[] }> {
  try { return await (await ghostFetch(`${BASE}/api/connectors/registry`)).json(); } catch { return { connectors: [], custom: [] }; }
}
export async function connectConnector(name: string, url = ""): Promise<{ ok: boolean; message?: string; error?: string }> {
  try {
    const r = await ghostFetch(`${BASE}/api/connectors/connect`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, url }) });
    return await r.json();
...
export async function getConnectorIndex(): Promise<ConnectorIndex> {
  try { return (await (await ghostFetch(`${BASE}/api/connectors/index`)).json()).index || {}; } catch { return {}; }
}
export async function indexConnector(name: string): Promise<{ ok: boolean; message?: string; error?: string }> {
  try {
    const r = await ghostFetch(`${BASE}/api/connectors/index`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) });
    return await r.json();
  } catch (e) { return { ok: false, error: String(e) }; }
}
export async function removeConnector(name: string): Promise<{ ok: boolean }> {
  try {
    const r = await ghostFetch(`${BASE}/api/connectors/remove`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) });
    return await r.json();
  } catch { return { ok: false }; }
}
export async function getTools(): Promise<{ name: string; desc: string; on: boolean }[]> {
  try { return (await (await ghostFetch(`${BASE}/api/tools`)).json()).tools || []; } catch { return []; }
}
export async function setCodex(model: string | null, effort?: string): Promise<Status> {
  await ghostFetch(`${BASE}/api/codex`, {
...
export async function setSttProvider(provider: string): Promise<Status> {
  await ghostFetch(`${BASE}/api/stt`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider }) });
  return getStatus();
}
export async function setElevenKey(key: string): Promise<Status> {
  await ghostFetch(`${BASE}/api/apikey/elevenlabs`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key }) });
  return getStatus();
}

export async function setLang(lang: string): Promise<void> {
  try { await ghostFetch(`${BASE}/api/lang`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lang }) }); } catch { /* ignore */ }
}

export async function setApiKey(key: string): Promise<boolean> {
  try {
    const r = await ghostFetch(`${BASE}/api/apikey`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key }) });
    return (await r.json()).set;
  } catch { return false; }
}
export async function getMeetings(): Promise<MeetingMeta[]> {
  try { return (await (await ghostFetch(`${BASE}/api/meetings`)).json()).meetings || []; } catch { return []; }
}
...
export async function getStorage(): Promise<Storage | null> {
  try { return await (await ghostFetch(`${BASE}/api/storage`)).json(); } catch { return null; }
}
export async function setStorage(path: string): Promise<{ ok: boolean; home?: string; message?: string; error?: string }> {
  try {
    const r = await ghostFetch(`${BASE}/api/storage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path }) });
    return await r.json();
  } catch (e) { return { ok: false, error: String(e) }; }
}
...
export async function getSttModel(): Promise<SttModel | null> {
  try { return await (await ghostFetch(`${BASE}/api/stt/model`)).json(); } catch { return null; }
}
...
export async function getTranslateLangs(): Promise<TransLang[]> {
  try { return (await (await ghostFetch(`${BASE}/api/translate/langs`)).json()).langs || []; } catch { return []; }
}
export async function translateText(text: string, target: string): Promise<string> {
  try { return (await (await ghostFetch(`${BASE}/api/translate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, target }) })).json()).text || ""; } catch { return ""; }
}
...
export function streamTranslate(
  ...
  (async () => {
    const r = await ghostFetch(`${BASE}/api/translate/stream`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function polishLine(text: string, meetingId = ""): Promise<PolishResult | null> {
  try {
    const r = await ghostFetch(`${BASE}/api/polish`, {
      method: "POST", headers: { "Content-Type": "application/json" },
...
export async function getGlossary(): Promise<GlossaryItem[]> {
  try { return (await (await ghostFetch(`${BASE}/api/glossary`)).json()).glossary || []; } catch { return []; }
}
...
export function sttWsUrl(meetingId = "", source = "mic"): string {
  ...
  if (TOKEN) qs.set("token", TOKEN);   // ws는 커스텀 헤더 불가 → 쿼리로 인증
  const q = qs.toString();
...
export async function getEngineInstall(): Promise<EngineInstall> {
  try { return await (await ghostFetch(`${BASE}/api/stt/engine/install`)).json(); } catch { return { state: "idle" }; }
}
export async function selectSttModel(kind: "local" | "cloud", modelId: string): Promise<{ ok: boolean; model?: string }> {
  try {
    const r = await ghostFetch(`${BASE}/api/stt/select`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind, model_id: modelId }) });
    return await r.json();
  } catch { return { ok: false }; }
}
...
export async function getUsage(): Promise<Usage | null> {
  try {
    const r = await ghostFetch(`${BASE}/api/usage`);
...
export async function setUsageBudget(usd: number): Promise<Usage | null> {
  try { return await (await ghostFetch(`${BASE}/api/usage/budget`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ usd }) })).json(); } catch { return null; }
}
export async function resetUsage(): Promise<Usage | null> {
  try { return await (await ghostFetch(`${BASE}/api/usage/reset`, { method: "POST" })).json(); } catch { return null; }
}
...
export async function transcribe(blob: Blob, meetingId = "", source = "mic"): Promise<TranscribeResult> {
  ...
  const r = await ghostFetch(`${BASE}/api/transcribe`, { method: "POST", body: fd });
...
export async function uploadAudio(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (TOKEN) xhr.setRequestHeader("x-ghost-token", TOKEN);
...
export async function ttsUrl(text: string): Promise<string> {
  const r = await ghostFetch(`${BASE}/api/tts`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
